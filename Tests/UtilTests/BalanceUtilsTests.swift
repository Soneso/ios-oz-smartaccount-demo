...
/// Tests for `formatStroopsAsXlm` in `BalanceUtils.swift`.
///
...
struct BalanceUtilsTests {
    ...
    // -------------------------------------------------------------------------
    // MARK: - formatStroopsAsXlm(Int64)
    // -------------------------------------------------------------------------

    @Test("Zero stroops formats as 0.0")
    func zeroStroops() {
        #expect(formatStroopsAsXlm(Int64(0)) == "0.0")
    }

    @Test("Exactly 1 XLM (10_000_000 stroops) formats as 1.0")
    func oneXlm() {
        #expect(formatStroopsAsXlm(Int64(10_000_000)) == "1.0")
    }

    @Test("100 XLM formats correctly")
    func oneHundredXlm() {
        #expect(formatStroopsAsXlm(Int64(1_000_000_000)) == "100.0")
    }

    @Test("0.5 XLM (5_000_000 stroops) formats as 0.5")
    func halfXlm() {
        #expect(formatStroopsAsXlm(Int64(5_000_000)) == "0.5")
    }
    ...
    func int64Min() {
        #expect(formatStroopsAsXlm(Int64.min) == "-922337203685.4775808")
    }
    ...
    func int64Max() {
        ...
        let result = formatStroopsAsXlm(Int64.max)
        #expect(result.hasPrefix("922337203685."))
        #expect(result.hasSuffix("7"))
    }

    // -------------------------------------------------------------------------
    // MARK: - formatStroopsAsXlm(String)
    // -------------------------------------------------------------------------

    @Test("String overload parses and formats valid stroops string")
    ...
    func int128BeyondInt64Range() {
        // 2^64 stroops, well above Int64.max; Int64-based formatters would
        // require sentinel substitution. The Int128 formatter preserves it.
        let stroops: Int128 = Int128(1) << 64
        let formatted = formatStroopsAsXlm(stroops)
        // 2^64 = 18_446_744_073_709_551_616 stroops = 1_844_674_407_370.9551616 XLM
    ...
    func int128MaxPositive() {
        ...
        // Divide by 10^7 stroops/XLM — insert the decimal point 7 digits from
        // the right:
        //   whole = 17_014_118_346_046_923_173_168_730_371_588
        //   frac  = 4_105_727
        let formatted = formatStroopsAsXlm(Int128.max)
    ...
    func int128MinNegative() {
        ...
        let formatted = formatStroopsAsXlm(Int128.min)
        #expect(formatted == "-17014118346046923173168730371588.4105728")
    }

    @Test("Int128 overload: very small positive (1 stroop) formats correctly")
    func int128OneStroop() {
        #expect(formatStroopsAsXlm(Int128(1)) == "0.0000001")
    }

    @Test("Int128 overload: very small negative (-1 stroop) formats correctly")
    func int128MinusOneStroop() {
        #expect(formatStroopsAsXlm(Int128(-1)) == "-0.0000001")
    }
    ...
    @Test("formatSmallestUnitsAsDecimal Int128 overload forwards to formatStroopsAsXlm")
    func smallestUnitsInt128Aliases() {
        let value: Int128 = 10_000_000
        #expect(formatSmallestUnitsAsDecimal(value) == formatStroopsAsXlm(value))
    }
}
